...
process_endo213216 <- function() {
  ...
  members <- utils::untar("/Volumes/Extreme SSD/02_endometriosis/GSE213216/GSE213216_RAW.tar", list = TRUE)
  ...
  out <- lapply(seq_len(nrow(map213)), function(i) {
    mtx <- read_10x_from_nested_tar("/Volumes/Extreme SSD/02_endometriosis/GSE213216/GSE213216_RAW.tar", map213$member[i], inner_prefix = sub("\\.tar\\.gz$", "", map213$member[i]))
    assign <- assign_cells_heca(mtx, centroid_mat)
    agg <- aggregate_target_counts(mtx, assign, target_genes)
